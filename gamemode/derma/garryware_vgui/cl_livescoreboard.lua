...
function PANEL:LabelRefresh( bForce )
	if !bForce and (CurTime() < (self.LabelMakeRefreshDelay + self.LabelMakeRefreshTime)) then return end
...
function PANEL:SortPlayerList()
	...
	while k <= #self.m_playerlist do
		if !IsValid( self.m_playerlist[k] ) then
		...
		if ply:IsWarePlayer() then
			...
			if !self.m_secondary_sort then
				if ply:GetAchieved() then
					if !(ply._cl_label.m_tpos == iWinStack) or !(ply._cl_label.m_tcol == 1) then
					...
					if !(ply._cl_label.m_tpos == iFailStack) or !(ply._cl_label.m_tcol == -1) then
				...
				if !(ply._cl_label.m_tpos == iWinStack) or !(ply._cl_label.m_tcol == 0) then
			...
			if !self.m_secondary_sort then
				...
				if !(ply._cl_label.m_tpos == iTheoStack) or !(ply._cl_label.m_tcol == 0) then

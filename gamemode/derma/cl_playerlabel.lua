...
function PANEL:Think()
	...
	if !IsValid(self.m_player) or !self.m_player:IsPlayer() then
		self:Remove()
		return
	end
	...
	if self.m_player:IsFirst() and !self.m_first then
		self.dFirstArrow:SetText( "1st" )
		self.dFirstArrow:UseLeft( false )
		self.m_first = !self.m_first

	elseif !self.m_player:IsFirst() and self.m_first then

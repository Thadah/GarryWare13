...
function WARE:Think()
  for _,v in pairs(self.Speakers) do
  	if v.AlarmSound and v.AlarmPitch then
  		...
  		v.AlarmSound:ChangePitch(v.AlarmPitch, 1)
...
function WARE:PropBreak(pl,prop)
  if !(pl:IsPlayer() and pl:IsWarePlayer()) then return end
  ...
  if prop.Speaker then
  	...
  	prop.Speaker:GetPhysicsObject():ApplyForceCenter((prop:GetPos() - pl:GetPos() - Vector(0,0,20)):GetNormalized() * math.random(10000,20000))
  end
end

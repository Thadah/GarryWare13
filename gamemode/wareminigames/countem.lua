...
function WARE:StartAction()
	...
	local howManyPossible = (!useX and self.Grid:Width() or self.Grid:Height())
	for i=1,howManyPossible do
		local myCopo = self.Grid:Get( (useX and whichAttic or i), (!useX and whichAttic or i) )
	...
	local extremaA = self.Grid:Get( (useX and whichAttic or 1), (!useX and whichAttic or 1) ):GetPos()
	local extremaB = self.Grid:Get( (useX and whichAttic or howManyPossible), (!useX and whichAttic or howManyPossible) ):GetPos()
...
function WARE:EntityTakeDamage(ent,info)
	...
	if !ent.CrateID then return end

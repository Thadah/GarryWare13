...
function WARE:PreEndAction()
	if GAMEMODE:GetCurrentPhase() == 1
		...
		while ((k <= #self.PlayersWhoDone) and !someoneAchieved) do
...
function WARE:EntityTakeDamage(ent,info)
	...
	if GAMEMODE:GetCurrentPhase() == 2 then
		if !IsValid( ent ) or !ent:IsPlayer() or !ent:IsWarePlayer() or ent.WARE_IsAttacker or ent:GetLocked() then return end
		if !IsValid( attacker ) or !attacker:IsPlayer() or !attacker:IsWarePlayer() or !attacker.WARE_IsAttacker then return end

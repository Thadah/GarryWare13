...
function WARE:Initialize()
	...
	for i,pos in ipairs(GAMEMODE:GetRandomPositions(numberSpawns, ENTS_ONCRATE)) do
		...
		local col1 = self.CrateColours[num]
		...
		prop:SetColor(Color(col1[1] * 100, col1[2] * 100, col1[3] * 100, 100))
		prop:SetRenderMode(RENDERMODE_TRANSALPHA)
		prop:SetHealth(100000)
		...
		timer.Simple(delay + (i-1), function()
			...
			prop:SetColor(Color(col1[1] * 255, col1[2] * 255, col1[3] * 255, 255))
			prop:SetHealth(100000)
			...
			timer.Simple(0.5, function()
				...
				local prop2 = self.Crates[i]
				if !(prop2 and prop2:IsValid()) then return end

				local col2 = self.CrateColours[num]

				prop:SetColor(Color(col2[1] * 100, col2[2] * 100, col2[3] * 100, 100))
			end)
		end)

...
function meta:ApplyLock( dontSendStatusMessage )
	if GAMEMODE:PhaseIsPrelude() then return false end
	if !self:IsWarePlayer() then return false end
	...
	if !(dontSendStatusMessage or false) then
...
function meta:RestoreDeath()
	if !self:IsSimDead() then return false end
...
function meta:Recoil( pitch, yaw )
	...
	if ( !SinglePlayer() and !IsFirstTimePredicted() ) then return end

--[[
	...
	Credits to:
		thelastpenguin for pon2.
		https://github.com/thelastpenguin/gLUA-Library/tree/master/pon2
--]]
...
if (SERVER) then
	...
	function netstream.Start(player, name, ...)
		...
		local dataTable = {...}
		local encodedData = pon2.encode(dataTable)
		...
		if (NS_DS_NAME and NS_DS_DATA and NS_DS_LENGTH) then
			...
			if (player.nsDataStreamName and player.nsDataStreamData) then
				...
				if (netstream.stored[player.nsDataStreamName]) then
					local bStatus, value = pcall(pon2.decode, player.nsDataStreamData)
	...
	net.Receive("NetStreamDS", function(length)
		...
		if (NS_DS_NAME and NS_DS_DATA and NS_DS_LENGTH) then
			if (netstream.stored[NS_DS_NAME]) then
				local bStatus, value = pcall(pon2.decode, NS_DS_DATA)

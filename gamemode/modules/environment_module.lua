...
local function FindBestEnvironmentByPlayerCount(list)
	...
	for _,v in ipairs(list) do
		if !lastDiff2 then -- "Good" room not found yet, find the best room that approaches most the player count
			if numPlayers>=v.MaxPlayers then
				if !lastDiff or numPlayers-v.MaxPlayers<lastDiff then
					best = v
					lastDiff = numPlayers-v.MaxPlayers
				end
			elseif numPlayers<v.MinPlayers then
				if !lastDiff or v.MinPlayers-numPlayers<lastDiff then
					best = v
		...
		if numPlayers>=v.MinPlayers and numPlayers<v.MaxPlayers then
			...
			if !lastDiff2 or diff<lastDiff2 then
				lastDiff2 = diff
				best = v
			end
		end
	...
	if ent and ent:IsValid() and ent:GetClass()=="func_wareroom" then
		...
		for _,v in pairs(ents.GetAll()) do
			...
			elseif v:GetClass()=="gmod_warelocation" then
				if !env.Locations[v:GetName()] then
...
function HasEnvironment(name)
	if name=="none" or name=="generic" or name=="" or !name then
		return true
	end
...
function FindEnvironment(name)
	if name=="none" and !GAMEMODE.CurrentEnvironment then
		name="generic"
	end
	
	if name=="none" then
		return GAMEMODE.CurrentEnvironment
	elseif !name or name=="" or name=="generic" then
		local list = {}

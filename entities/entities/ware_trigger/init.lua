...
function ENT:Setup(mins, maxs, callback, filter, iswhitelist)
	self:SetNWVector("mins", mins)
	self:SetNWVector("maxs", maxs)
	self.mins = mins
	self.maxs = maxs
	...
	self:SetNWVector("vmax", self.vmax)
...
function ENT:IsInBox(pos)
	...
	local proj_length = diff:Dot(self.F)
	local proj_width  = diff:Dot(self.R)
	local proj_height = diff:Dot(self.U)
...
function ENT:Think(ent)
	...
	for _,v in pairs(ents.FindInBox(pos-self.vmax, pos + self.vmax)) do
		...
		if cond and v != self and v != self:GetParent() and self:IsInBox(v:GetPos()) then
